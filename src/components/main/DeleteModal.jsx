...
const EditModal = () => {
	...
	useEffect(() => {
		...
		document.addEventListener("openDeleteModal", openDeleteModal);

		return () => {
			document.removeEventListener("openDeleteModal", openDeleteModal);
		};
	}, []);

	function handleDelete() {
		sunnah
			.delete(`${data.path}/${data.id}`)
			.then(({ data: resp }) => {
				if (resp.status === 200) {
